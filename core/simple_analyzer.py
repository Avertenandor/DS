    ...
    def _process_transfer_logs(self, logs: List) -> List[Dict]:
        ...
        for log in logs:
            try:
                ...
                if len(log['topics']) < 3:
                    logger.warning("⚠️ Недостаточно topics в Transfer логе")
                    continue
                
                # Безопасное извлечение адресов
                from_topic = log['topics'][1]
                to_topic = log['topics'][2]
                
                # Конвертируем в hex строку если это bytes
                if isinstance(from_topic, bytes):
                    from_hex = from_topic.hex()
                else:
                    from_hex = from_topic.hex() if hasattr(from_topic, 'hex') else str(from_topic)
                ...
                # Извлекаем последние 40 символов (20 байт = адрес)
                from_address = self.w3.to_checksum_address("0x" + from_hex[-40:])
                to_address = self.w3.to_checksum_address("0x" + to_hex[-40:])
                
                # Безопасное декодирование amount
                amount_data = log['data']
                if isinstance(amount_data, bytes):
                    ...
                    amount_hex = str(amount_data)
                
                # Убираем префикс 0x если есть
                if amount_hex.startswith('0x'):
                    amount_hex = amount_hex[2:]
                
                # Конвертируем в int, обрабатывая случай пустой строки
                if amount_hex:
                ...
                transfer = {
                    ...
                    'transaction_hash': log['transactionHash'].hex() if hasattr(log['transactionHash'], 'hex') else str(log['transactionHash'])
                }
                ...
                logger.debug(f"🔍 Проблемный лог: {log}")
                continue
