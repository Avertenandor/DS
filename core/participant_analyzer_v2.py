"""
Модуль: Анализатор участников PLEX Dynamic Staking (Версия 2)
Описание: Анализ активности участников с интегрированными модулями категоризации и eligibility
...
logger = get_logger("PLEX_ParticipantAnalyzer_V2")
...
class ParticipantAnalyzer:
    ...
    def __init__(self):
        ...
        logger.info("🔧 Анализатор участников V2 инициализирован с интегрированными модулями")
    ...
    def analyze_participants(self, 
        ...
        try:
            # Получаем все swap события в указанном диапазоне
    ...
    def get_participants_for_rewards(self, min_tier: str = "Bronze") -> List[ParticipantData]:
        """Получение участников, квалифицированных для наград"""
        try:
            tier_priority = {"Bronze": 1, "Silver": 2, "Gold": 3, "Platinum": 4}
            min_priority = tier_priority.get(min_tier, 1)
            
            qualified = []
            for participant in self.participants.values():
                if participant.is_qualified:
                    participant_priority = tier_priority.get(participant.reward_tier, 0)
                    if participant_priority >= min_priority:
                        qualified.append(participant)
            
            # Сортируем по eligibility score
            qualified.sort(key=lambda x: x.eligibility_score, reverse=True)
            
            return qualified
            
        except Exception as e:
            logger.error(f"❌ Ошибка получения участников для наград: {e}")
            return []
    ...
    def close(self):
        ...
        try:
            ...
            logger.info("🔐 Анализатор участников закрыт")
        except Exception as e:
            logger.error(f"❌ Ошибка закрытия анализатора: {e}")
