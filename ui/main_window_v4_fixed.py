...
class PLEXStakingMainWindow:
    ...
    def _create_header(self) -> None:
        ...
        self.title_label = self.theme.create_styled_label(
            self.header_frame,
            "💎 PLEX Dynamic Staking Manager",
            'title'
        )
        self.title_label.pack(side='left', padx=20, pady=10)
        
        # Статус подключения
        self.status_frame = self.theme.create_styled_frame(self.header_frame, 'secondary')
        ...
        self.connection_indicator.pack(side='right', padx=(0, 10))
        
        self.version_label = self.theme.create_styled_label(
            self.status_frame,
            "v4.0.0",
            'secondary'
        )
        self.version_label.pack(side='right', padx=(0, 20))
