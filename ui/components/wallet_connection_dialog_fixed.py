"""
PLEX Dynamic Staking Manager - Wallet Connection Dialog (Fixed)
Исправленный диалог для подключения кошелька по seed фразе или приватному ключу.
...
class WalletConnectionDialog(ctk.CTkToplevel):
    ...
    def _create_interface(self) -> None:
        ...
        # Статус (создаем перед input fields)
    ...
    def _connect_wallet(self) -> None:
        ...
        try:
            ...
            if method == "seed":
                ...
                self._update_status("✅ Кошелек подключен через приватный ключ")
...
def show_wallet_connection_dialog(parent, on_wallet_connected: Optional[Callable] = None) -> Optional[Dict[str, Any]]:
    """
