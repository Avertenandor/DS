    ...
    def _try_blockchain_connection(self) -> None:
        ...
        def connect():
            try:
                ...
                if self.staking_manager and hasattr(self.staking_manager, 'test_connection'):
                    success = self.staking_manager.test_connection()
                    if success:
                        self.connection_status = "connected"
                        self.root.after_idle(lambda: self._update_header_status("node", "ready", "Подключена"))
                        self.root.after_idle(lambda: self._update_activity("✅ Подключен к BSC"))
                    else:
                        self.connection_status = "error"
